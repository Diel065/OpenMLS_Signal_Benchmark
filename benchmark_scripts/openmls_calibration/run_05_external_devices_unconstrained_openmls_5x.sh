#!/usr/bin/env bash
#
# run_05_external_devices_unconstrained_openmls_5x.sh
#
# External-device-only OpenMLS regular benchmark. The historical filename and
# cal05_ext_unconstrained run-id prefix are retained for compatibility with
# existing automation; this is not a container baseline or resource campaign.
#
# Design:
#   * Production defaults use one ascending run at exact plateaus
#     4,64,256,512,1024 with four update and application rounds.
#   * No Docker resource experiment, affinity, or container profiling is run.
#   * ONLY external workers are profiled. All Docker workers run with
#     --disable-container-profiling, so they are pure virtual group-size
#     increasers; the external devices are the only profiled entities
#     (via --external-coverage-lane).
#   * After every run the script hard-verifies that alloc_bytes AND
#     cpu_process_ns are present and non-zero for all three external devices.
#
...
# ---- output location ------------------------------------------------------
# Write all run outputs to /dev/sda2 (the root filesystem, which has the free
# space) rather than the small /dev/shm tmpfs. OUTPUT_ROOT must live on a real
# disk with space. The generated docker-compose bind-mounts "./<output_dir>"
# relative to the stack dir, so --output-dir is passed as a path RELATIVE to
# OpenMLS_containerized (absolute paths would break that bind mount).
...
# Luckfox has only 256 MiB RAM. It may be evicted after an evidenced failure;
# the Raspberry Pis remain required for a scientifically usable run.
ATTRITION_ALLOWED_WORKER_IDS="${ATTRITION_ALLOWED_WORKER_IDS:-pico-plus-00001}"
SCENARIO="${SCENARIO:-external-device-openmls-regular}"
...
# ---- external-device workload with a per-plateau coverage floor -----------
WORKERS="${WORKERS:-1024}"
MAX_SIZE="${MAX_SIZE:-1024}"
# Size 4 is the smallest plateau containing the leader plus all three devices.
MIN_SIZE="${MIN_SIZE:-4}"
...
run_external() {
  local iter="$1"
  local run_id="cal05_ext_unconstrained_openmls_i${iter}_${DATE_TAG}"
  ...
  echo ""
  echo "===== [05/external-regular] OpenMLS iteration $iter/$N run_id=$run_id ====="
  echo "  workers=$WORKERS max_size=$MAX_SIZE step_size=$STEP_SIZE payloads=$PAYLOAD_SIZES"
  ...
  echo "----- external-device metric verification (alloc_bytes + cpu_process_ns) -----"
...
if [ "$STATUS" -eq 0 ]; then
  echo ""
  echo "All $N OpenMLS external-device runs completed; alloc_bytes + cpu_process_ns verified for all three devices."
else
  echo ""
  echo "One or more OpenMLS external-device runs failed; see output above." >&2
fi
exit "$STATUS"
