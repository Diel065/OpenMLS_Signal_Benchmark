"""
Failure classification and tracking for Signal resource experiments.
...
context (group size, operation, phase) when a worker fails.

Adapted for Signal: docker-cgroup memory model (no app-heap-budget allocator).
Signal uses conversation_size/participant_count instead of member_count/epoch.
"""
...
def classify_worker_failure_from_resource_summary(
    ...
    if not logical_client_id and physical_worker_id.startswith("worker-"):
        logical_client_id = physical_worker_id.removeprefix("worker-")

    info = WorkerFailureInfo(
        worker_id=str(resource_summary.get("worker_id") or logical_client_id),
        physical_worker_id=physical_worker_id,
        logical_client_id=logical_client_id,
        container_name=str(resource_summary.get("container_name") or physical_worker_id),
        container_id=str(resource_summary.get("container_id") or ""),
...
def _benchmark_operation_family(operation: str, phase: str = "") -> str:
    ...
    if operation in ("signal_session_establish", "session_establish") or phase == "session_establish":
        return "signal_session_establish"
    if operation in ("signal_application_message_create", "message_create") or phase == "application_create":
        return "signal_application_message_create"
    if operation in ("signal_application_message_receive", "message_receive") or phase == "application_receive":
        return "signal_application_message_receive"
    if operation in ("signal_prekey_bundle_create", "prekey_create"):
        return "signal_prekey_bundle_create"
    if operation in ("signal_prekey_maintenance", "prekey_maintenance"):
        return "signal_prekey_maintenance"
    if operation in ("signal_participant_remove", "participant_remove"):
        return "signal_participant_remove"
    return ""
    ...
    try:
        ...
        for row in rows:
            ...
            cursor = {
                ...
                "member_count": _safe_int(row.get("member_count") or row.get("conversation_size")),
...
def collect_worker_failures_from_artifacts(
    ...
    for summary in resource_summaries:
        ...
        client_id = resource_failure.logical_client_id or resource_failure.worker_id
        existing = by_client.get(client_id)
        if existing is not None:
            merge_resource_summary_into_failure(existing, summary)
            if resource_class in ("hard_ram_oom_kill", "hard_container_exit"):
                existing.failure_class = resource_class
            continue
        ...
        if not resource_failure.failure_detail:
            resource_failure.failure_detail = (
                f"container_status={resource_failure.last_container_status or 'unknown'} "
                f"exit_code={resource_failure.container_exit_code} "
                f"oom_killed={resource_failure.container_oom_killed} "
                f"memory_events_oom_kill={resource_failure.memory_events_oom_kill}"
            )
        if not resource_failure.failure_action:
            resource_failure.failure_action = "stop_run"
        if resource_failure.failure_timestamp_ns == 0:
            resource_failure.failure_timestamp_ns = time.time_ns()
