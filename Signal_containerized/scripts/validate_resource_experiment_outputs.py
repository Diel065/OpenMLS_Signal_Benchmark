#!/usr/bin/env python3
"""
Validate Signal resource experiment output files.
...
Signal supports docker-cgroup memory and app-heap-budget allocator runs.
"""
...
class Validator:
    ...
    def validate(self) -> bool:
        try:
            ...
            self._check_docker_cgroup_run()
            self._check_parallel_sweep_run()
            self._check_app_heap_budget_run()
    ...
    def _check_preflight_status(self):
        ...
        has_fail = any(s.strip().lower() == "fail" for s in statuses if s.strip())
    ...
    def _check_selected_profile(self):
        ...
        is_parallel_sweep = sweep_kind in ("ram_sweep", "cpu_quota_sweep", "ram_app_heap_sweep")
    ...
    def _check_docker_cgroup_run(self):
        """Check docker-cgroup memory model runs (Signal has no app-heap-budget).

        Signal uses docker-cgroup for all memory constraints. This validates
        that docker-cgroup runs are correctly configured.
        """
        status_rows = self._read_rows(os.path.join(self.run_dir, "run_status.csv"))
        if not status_rows:
            return
        status = status_rows[0]
        experiment_kind = status.get("experiment_kind", "")
        is_embedded = (
            status.get("resource_experiment") == "embedded-budget-singleton"
            or experiment_kind == "embedded_budget_singleton"
        )
        if not is_embedded:
            return
        ...
        mem_model = status.get("memory_model", "")
        if mem_model and mem_model not in ("docker-cgroup", "app-heap-budget"):
            self.error(f"run_status.csv: embedded-budget run must use docker-cgroup or app-heap-budget memory model, got '{mem_model}'")
    ...
    def _check_parallel_sweep_run(self):
        ...
        if is_ram:
            for r in profiled_singletons:
                if r.get("memory_model") not in ("docker-cgroup", "app-heap-budget"):
                    self.error(f"RAM sweep worker {r.get('container_name')} must use docker-cgroup memory model")
            ...
            if group_creators:
                gc = group_creators[0]
                gc_mem = (gc.get("memory_limit") or gc.get("app_heap_budget") or "").strip().lower()
                if gc_mem and parse_memory_to_bytes(gc_mem) < 1024 * 1024 * 1024:
                    self.error(
                        f"RAM sweep group creator memory limit '{gc_mem}' is too small; "
                        f"must be the largest memory worker"
                    )
        ...
        if is_cpu:
            ...
            for r in profiled_singletons:
                mem_model = r.get("memory_model", "").strip()
                if mem_model and mem_model not in ("docker-cgroup", "app-heap-budget"):
                    self.error(f"CPU sweep worker {r.get('container_name')} memory model '{mem_model}' not recognized")
    ...
    def _check_app_heap_budget_run(self):
        profiles = self._read_rows(os.path.join(self.run_dir, "resource_profiles.csv"))
        app_profiles = [row for row in profiles if row.get("memory_model") == "app-heap-budget"]
        if not app_profiles:
            return

        for row in app_profiles:
            if not row.get("app_heap_budget"):
                self.error("resource_profiles.csv: app-heap-budget profile missing app_heap_budget")
            if _safe_int(row.get("app_heap_budget_bytes")) <= 0:
                self.error("resource_profiles.csv: app-heap-budget profile missing app_heap_budget_bytes")

        status_rows = self._read_rows(os.path.join(self.run_dir, "run_status.csv"))
        if status_rows and status_rows[0].get("memory_model") != "app-heap-budget":
            self.error("run_status.csv: app-heap-budget run must record memory_model=app-heap-budget")

        assignments = self._read_rows(os.path.join(self.run_dir, "worker_resource_assignments.csv"))
        app_assignments = [row for row in assignments if row.get("memory_model") == "app-heap-budget"]
        if not app_assignments:
            self.error("worker_resource_assignments.csv: no app-heap-budget assignments found")
        for row in app_assignments:
            if _safe_int(row.get("app_heap_budget_bytes")) <= 0:
                self.error("worker_resource_assignments.csv: app-heap-budget assignment missing budget bytes")

        events_path = os.path.join(self.run_dir, "events.csv")
        if not os.path.exists(events_path):
            self.error("app-heap-budget run missing events.csv")
            return
        with open(events_path, newline="", encoding="utf-8") as handle:
            ...
            if missing:
                self.error(f"events.csv: missing app heap columns {sorted(missing)}")
                return
            rows = list(reader)
        heap_rows = [row for row in rows if row.get("memory_model") == "app-heap-budget"]
        if not heap_rows:
            self.error("events.csv: no rows record memory_model=app-heap-budget")
        if heap_rows and not any(_safe_int(row.get("app_heap_budget_bytes")) > 0 for row in heap_rows):
            self.error("events.csv: app-heap-budget rows do not record app_heap_budget_bytes")
...
def main():
    parser = argparse.ArgumentParser(
        description="Validate Signal resource experiment output files"
    )
    parser.add_argument(
        "run_dir",
