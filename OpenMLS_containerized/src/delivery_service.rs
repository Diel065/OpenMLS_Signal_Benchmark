...
type CommitInboxQueue = Arc<Mutex<VecDeque<CommitDeliveryRecord>>>;
...
pub struct PendingCommitDelivery {
    pub id: String,
    pub group_id: String,
    pub epoch: u64,
    pub sender: String,
...
pub struct DeliveryService {
    ...
    commit_inboxes: RwLock<HashMap<String, CommitInboxQueue>>,
...
impl DeliveryService {
    pub fn new() -> Self {
        ...
        Self::with_delivery_mode(delivery_mode)
    }

    fn with_delivery_mode(delivery_mode: CommitDeliveryMode) -> Self {
        Self {
        ...
        match self.delivery_mode {
            CommitDeliveryMode::PerRecipient => {
                for recipient in recipients {
                    let delivery_id = commit_delivery_id(group_id, epoch + 1, sender, &recipient);
                    self.commit_inbox_queue(&recipient)
                        .lock()
                        .unwrap()
                        .push_back(CommitDeliveryRecord {
                            id: delivery_id,
    ...
    pub fn fetch_commit(&self, recipient: &str) -> Option<Vec<u8>> {
        self.fetch_next_pending_commit(recipient)
            .map(|delivery| delivery.commit_bytes.as_ref().clone())
    }

    pub fn fetch_pending_commit_delivery(&self, recipient: &str) -> Option<PendingCommitDelivery> {
        self.fetch_next_pending_commit(recipient)
            .map(|delivery| PendingCommitDelivery {
                id: delivery.id,
                group_id: delivery.group_id,
                epoch: delivery.epoch,
                sender: delivery.sender,
                commit_hex: hex::encode(delivery.commit_bytes.as_ref()),
            })
    }

    pub fn ack_commit(&self, recipient: &str, delivery_id: &str) -> bool {
        match self.delivery_mode {
            CommitDeliveryMode::PerRecipient => {
                let Some(queue) = self.commit_inboxes.read().unwrap().get(recipient).cloned()
                else {
                    return false;
                };
                ...
                queue.retain(|delivery| delivery.id != delivery_id);
                before != queue.len()
            }
            CommitDeliveryMode::GroupLog => {
                self.ack_group_log_commit_compat(recipient, delivery_id)
            }
        }
    }
    ...
    fn commit_inbox_queue(&self, recipient: &str) -> CommitInboxQueue {
        if let Some(queue) = self.commit_inboxes.read().unwrap().get(recipient).cloned() {
            return queue;
        }

        let mut inboxes = self.commit_inboxes.write().unwrap();
        inboxes
            .entry(recipient.to_string())
            .or_insert_with(|| Arc::new(Mutex::new(VecDeque::new())))
            .clone()
    ...
    fn fetch_next_pending_commit(&self, recipient: &str) -> Option<CommitDeliveryRecord> {
        match self.delivery_mode {
            CommitDeliveryMode::PerRecipient => self.peek_commit_inbox(recipient),
            CommitDeliveryMode::GroupLog => self.fetch_group_log_commit_compat(recipient),
        }
    }

    fn peek_commit_inbox(&self, recipient: &str) -> Option<CommitDeliveryRecord> {
        let queue = self
            .commit_inboxes
            .read()
            .unwrap()
            .get(recipient)
            .cloned()?;
        let delivery = queue.lock().unwrap().front().cloned();
        delivery
    }

    fn fetch_group_log_commit_compat(&self, recipient: &str) -> Option<CommitDeliveryRecord> {
        let groups = self.group_commit_log.read().unwrap();
        let cursors = self.commit_cursors.lock().unwrap();
        let mut selected: Option<(String, u64, String, MessageBytes)> = None;
        ...
        for (group_id, commits) in groups.iter() {
            ...
            for commit in commits {
                ...
                let should_replace = selected
                    .as_ref()
                    .map(|(_, selected_epoch, _, _)| commit.epoch < *selected_epoch)
                    .unwrap_or(true);
        ...
        let (group_id, epoch, sender, bytes) = selected?;
        Some(CommitDeliveryRecord {
            id: commit_delivery_id(&group_id, epoch, &sender, recipient),
            group_id,
            epoch,
            sender,
            commit_bytes: bytes,
    ...
    fn ack_group_log_commit_compat(&self, recipient: &str, delivery_id: &str) -> bool {
        let groups = self.group_commit_log.read().unwrap();

        for (group_id, commits) in groups.iter() {
            for commit in commits {
                if !commit.recipients.iter().any(|id| id == recipient) {
                    continue;
                }

                if commit_delivery_id(group_id, commit.epoch, &commit.sender, recipient)
                    != delivery_id
                {
                    continue;
                }
                ...
                let cursor = cursors
                    .entry((recipient.to_string(), group_id.clone()))
                    .or_insert(0);
                *cursor = (*cursor).max(commit.epoch);
                return true;
            }
        }
...
fn commit_delivery_id(group_id: &str, epoch: u64, sender: &str, recipient: &str) -> String {
    format!("{group_id}:{epoch}:{sender}:{recipient}")
}
...
mod tests {
    ...
    #[test]
    fn per_recipient_commit_fetch_replays_until_ack() {
        let ds = DeliveryService::with_delivery_mode(CommitDeliveryMode::PerRecipient);
        ds.put_group_state("group-1", 0, vec!["alice".to_string()])
            .unwrap();
        ds.publish_group_commit("group-1", "alice", 0, b"commit-1".to_vec())
            .unwrap();

        let first = ds
            .fetch_pending_commit_delivery("alice")
            .expect("first pending commit");
        let second = ds
            .fetch_pending_commit_delivery("alice")
            .expect("replayed pending commit");

        assert_eq!(first.id, second.id);
        assert_eq!(first.commit_hex, hex::encode(b"commit-1"));
        assert!(ds.ack_commit("alice", &first.id));
        assert!(ds.fetch_pending_commit_delivery("alice").is_none());
        assert!(!ds.ack_commit("alice", &first.id));
    }

    #[test]
    fn group_log_pending_commits_are_state_based() {
        let ds = DeliveryService::with_delivery_mode(CommitDeliveryMode::GroupLog);
        ds.put_group_state("group-1", 0, vec!["alice".to_string()])
            .unwrap();
        ds.publish_group_commit("group-1", "alice", 0, b"commit-1".to_vec())
            .unwrap();

        let first = ds.fetch_pending_commits("alice", 0);
        let second = ds.fetch_pending_commits("alice", 0);

        assert_eq!(first.len(), 1);
        assert_eq!(first, second);
        assert_eq!(first[0].epoch, 1);
        assert_eq!(first[0].commit_hex, hex::encode(b"commit-1"));
        assert!(ds.fetch_pending_commits("alice", 1).is_empty());
    }
}
