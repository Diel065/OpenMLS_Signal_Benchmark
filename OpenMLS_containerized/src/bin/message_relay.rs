...
async fn main() -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route(
            "/application-message/{recipient}/pending",
            get(fetch_pending_application_message),
        )
        .route(
            "/application-message/{recipient}/ack/{message_id}",
            post(ack_application_message),
        )
        .with_state(state);
...
async fn fetch_pending_application_message(
    State(state): State<SharedRelay>,
    Path(recipient): Path<String>,
) -> Response {
    let metrics = state.metrics().start("fetch_pending_application_message");
    let response = match state.fetch_pending_application_message(&recipient) {
...
async fn ack_application_message(
    State(state): State<SharedRelay>,
    Path((recipient, message_id)): Path<(String, String)>,
) -> Response {
    let metrics = state.metrics().start("ack_application_message");
    let acked = state.ack_application_message(&recipient, &message_id);
    let response = if acked {
        StatusCode::OK.into_response()
    } else {
        (
            StatusCode::OK,
            "application message was already acked or absent",
        )
            .into_response()
    };
    metrics.finish_http_status(response.status().as_u16());
