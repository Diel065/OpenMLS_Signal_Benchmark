...
async fn main() -> Result<()> {
    ...
    let app = Router::new()
        ...
        .route("/commit/{recipient}/pending", get(fetch_pending_commit))
        .route("/commit/{recipient}/ack/{delivery_id}", post(ack_commit))
...
async fn fetch_pending_commit(
    State(state): State<SharedDs>,
    Path(recipient): Path<String>,
) -> Response {
    let metrics = state.metrics().start("fetch_pending_commit");
    let response = match state.fetch_pending_commit_delivery(&recipient) {
        Some(delivery) => Json(delivery).into_response(),
        None => StatusCode::NOT_FOUND.into_response(),
    };
    metrics.finish_http_status(response.status().as_u16());
    response
...
async fn ack_commit(
    State(state): State<SharedDs>,
    Path((recipient, delivery_id)): Path<(String, String)>,
) -> Response {
    let metrics = state.metrics().start("ack_commit");
    let acked = state.ack_commit(&recipient, &delivery_id);
    let response = if acked {
        StatusCode::OK.into_response()
    } else {
        (
            StatusCode::OK,
            "commit delivery was already acked or absent",
        )
            .into_response()
    };
    metrics.finish_http_status(response.status().as_u16());
