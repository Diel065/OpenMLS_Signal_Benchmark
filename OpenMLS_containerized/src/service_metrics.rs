...
mod tests {
    ...
    #[test]
    fn http_not_found_counts_as_visible_error() {
        let metrics = ServiceMetrics::new();
        metrics.start("fetch_commit").finish_http_status(404);

        let snapshot = metrics.snapshot();
        let endpoint = snapshot.endpoints.get("fetch_commit").unwrap();

        assert_eq!(endpoint.request_count, 1);
        assert_eq!(endpoint.error_count, 1);
        assert_eq!(endpoint.non_success_count, 1);
        assert_eq!(endpoint.client_error_count, 1);
        assert_eq!(endpoint.server_error_count, 0);
        assert_eq!(endpoint.not_found_count, 1);
    }
}
