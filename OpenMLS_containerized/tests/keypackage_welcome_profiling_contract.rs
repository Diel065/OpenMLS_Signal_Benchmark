...
fn canonical_key_package_and_welcome_receive_metadata_are_stable() {
    ...
    assert!(
        !kp_totals.is_empty(),
        "must have key_package_create_total_local spans"
    );
    for total in &kp_totals {
        ...
        assert!(
            total["artifact_size_bytes"].as_u64().unwrap_or(0) > 0,
    ...
    assert_eq!(
        wr_totals.len(),
        1,
        "exactly one welcome_receive_total_local"
    );
    ...
    assert!(
        !wr_events.is_empty(),
        "join_from_welcome child spans must carry operation_family"
    );
    ...
    for span in &[
        ...
        for f in &found {
            assert!(
                f["operation_family"].as_str().is_some(),
                "span {} must have operation_family",
                span
            );
        }
    }
    ...
    for f in events
        .iter()
        .filter(|e| e["op"] == "join_from_welcome_deserialize_welcome")
    {
        assert!(
            f["welcome_bytes"].as_u64().unwrap_or(0) > 0,
            "deserialize must have welcome_bytes"
        );
        assert!(
            f["welcome_recipient_count"].as_u64().is_some(),
            "must have welcome_recipient_count"
        );
    }
    // ratchet_tree_parse_and_validate should have ratchet_tree_bytes
    for f in events
        .iter()
        .filter(|e| e["op"] == "join_from_welcome.ratchet_tree_parse_and_validate")
    {
        assert!(
            f["ratchet_tree_bytes"].as_u64().is_some(),
            "must have ratchet_tree_bytes"
        );
    }
    // group_state_build should have member_count, tree metadata
    for f in events
        .iter()
        .filter(|e| e["op"] == "join_from_welcome.group_state_build")
    {
        assert!(
            f["member_count_before"].as_u64().is_some(),
            "must have member_count_before"
        );
        assert!(
            f["tree_node_count"].as_u64().is_some(),
            "must have tree_node_count"
        );
        assert!(f["tree_size"].as_u64().is_some(), "must have tree_size");
    }
    // total span should have everything
    assert!(
        total["welcome_bytes"].as_u64().unwrap_or(0) > 0,
        "total must have welcome_bytes"
    );
    assert!(
        total["ratchet_tree_bytes"].as_u64().is_some(),
        "total must have ratchet_tree_bytes"
    );
    assert!(
        total["welcome_recipient_count"].as_u64().is_some(),
        "total must have welcome_recipient_count"
    );
